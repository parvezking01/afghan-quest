...
@section('content')
    ...
    <div class="grid grid-cols-2 md:grid-cols-5 gap-4 mb-6">
        <div class="bg-white dark:bg-gray-800 rounded-2xl p-4 border border-gray-100 dark:border-gray-700 text-center">
            <p class="text-2xl font-black text-gray-700 dark:text-white">{{ $totalBookings ?? \App\Models\Booking::count() }}
            </p>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-2xl p-4 border border-gray-100 dark:border-gray-700 text-center">
            <p class="text-2xl font-black text-yellow-600 dark:text-yellow-400">
                {{ $pendingCount ?? \App\Models\Booking::where('status', 'pending')->count() }}</p>
            <p class="text-gray-400 dark:text-gray-500 text-xs">⏳ در انتظار</p>
        </div>
        <div class="bg-white dark:bg-gray-800 rounded-2xl p-4 border border-gray-100 dark:border-gray-700 text-center">
            <p class="text-2xl font-black text-blue-600 dark:text-blue-400">
                {{ $contactedCount ?? \App\Models\Booking::where('status', 'contacted')->count() }}</p>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-2xl p-4 border border-gray-100 dark:border-gray-700 text-center">
            <p class="text-2xl font-black text-green-600 dark:text-green-400">
                {{ $confirmedCount ?? \App\Models\Booking::where('status', 'confirmed')->count() }}</p>
            <p class="text-gray-400 dark:text-gray-500 text-xs">✅ تایید شده</p>
        </div>
        <div class="bg-white dark:bg-gray-800 rounded-2xl p-4 border border-gray-100 dark:border-gray-700 text-center">
            <p class="text-2xl font-black text-red-500 dark:text-red-400">
                {{ $cancelledCount ?? \App\Models\Booking::where('status', 'cancelled')->count() }}</p>
            ...
            <table class="w-full">
                <thead>
                    <tr class="bg-gray-50 dark:bg-gray-700/50">
                        <th class="text-right py-4 px-3 text-xs font-bold text-gray-400 dark:text-gray-500 uppercase">کد
                            رزرو</th>
                        <th class="text-right py-4 px-3 text-xs font-bold text-gray-400 dark:text-gray-500 uppercase">مهمان
                        </th>
                        <th class="text-right py-4 px-3 text-xs font-bold text-gray-400 dark:text-gray-500 uppercase">نوع
                        </th>
                        <th class="text-right py-4 px-3 text-xs font-bold text-gray-400 dark:text-gray-500 uppercase">جزئیات
                        </th>
                        <th class="text-right py-4 px-3 text-xs font-bold text-gray-400 dark:text-gray-500 uppercase">تاریخ
                        </th>
                        <th class="text-right py-4 px-3 text-xs font-bold text-gray-400 dark:text-gray-500 uppercase">مبلغ
                        </th>
                        <th class="text-right py-4 px-3 text-xs font-bold text-gray-400 dark:text-gray-500 uppercase">وضعیت
                        </th>
                        <th class="text-right py-4 px-3 text-xs font-bold text-gray-400 dark:text-gray-500 uppercase">عملیات
                        </th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-gray-50 dark:divide-gray-700">
                    @forelse($bookings as $booking)
                        <tr class="hover:bg-gray-50/50 dark:hover:bg-gray-700/50">
                            <td class="py-4 px-3">
                                <span
                                    class="text-sm font-mono font-bold text-gray-600 dark:text-gray-300">{{ $booking->booking_number }}</span>
                            </td>
                            <td class="py-4 px-3">
                            ...
                            <td class="py-4 px-3">
                                <span
                                    class="px-3 py-1 rounded-lg text-xs font-bold {{ $booking->booking_type === 'package' ? 'bg-purple-50 dark:bg-purple-900/30 text-purple-600 dark:text-purple-400' : 'bg-blue-50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400' }}">
                            ...
                            <td class="py-4 px-3">
                                ...
                                @if ($booking->room)
                                    <p class="text-xs text-gray-400 dark:text-gray-500">🛏️ {{ $booking->room->room_type }}
                                    </p>
                                @endif
                                @if ($booking->travel_date)
                                    <p class="text-xs text-gray-400 dark:text-gray-500">📅 سفر:
                                ...
                                @if ($booking->guest_message)
                                    <p class="text-xs text-gray-400 dark:text-gray-500 mt-1">💬
                                        {{ Str::limit($booking->guest_message, 50) }}</p>
                                @endif
                            </td>
                                ...
                                <div class="flex gap-1">
                                    ...
                                    @if ($booking->status === 'confirmed')
                                        <form action="{{ route('admin.bookings.status', $booking) }}" method="POST">
                                            ...
                                            <button
                                                class="bg-purple-50 dark:bg-purple-900/30 text-purple-600 dark:text-purple-400 p-2 rounded-lg hover:bg-purple-100 dark:hover:bg-purple-900/50 transition-colors"
                                                title="تکمیل شد">
