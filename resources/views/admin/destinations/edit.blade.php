        ...
        <form action="{{ route('admin.destinations.update', $destination) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="grid md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="text" name="name_en" value="{{ old('name_en', $destination->name_en) }}" class="w-full px-4 py-3 border border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-700 dark:text-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 text-left" dir="ltr">
                </div>
                ...
                <div>
                    ...
                    <select name="difficulty_level" class="w-full px-4 py-3 border border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-700 dark:text-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500" required>
                        <option value="easy" {{ $destination->difficulty_level === 'easy' ? 'selected' : '' }}>🟢 آسان</option>
                        <option value="moderate" {{ $destination->difficulty_level === 'moderate' ? 'selected' : '' }}>🟡 متوسط</option>
                        <option value="challenging" {{ $destination->difficulty_level === 'challenging' ? 'selected' : '' }}>🔴 سخت</option>
                    </select>
                </div>

                <div>
                    <label class="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-2">بهترین فصل</label>
                    <input type="text" name="best_season" value="{{ old('best_season', $destination->best_season) }}" class="w-full px-4 py-3 border border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-700 dark:text-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500">
                </div>

                <div>
                    <label class="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-2">مدت بازدید</label>
                    <input type="text" name="estimated_visit_duration" value="{{ old('estimated_visit_duration', $destination->estimated_visit_duration) }}" class="w-full px-4 py-3 border border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-700 dark:text-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500">
                </div>

                <div>
                    <label class="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-2">ترتیب نمایش</label>
                    <input type="number" name="display_order" value="{{ old('display_order', $destination->display_order) }}" class="w-full px-4 py-3 border border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-700 dark:text-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500">
                </div>
            </div>
            ...
            <div class="mt-6">
                ...
                <textarea name="description" rows="4" class="w-full px-4 py-3 border border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-700 dark:text-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500" required>{{ old('description', $destination->description) }}</textarea>
            </div>
            ...
            <div class="mt-6">
                ...
                <textarea name="description_en" rows="4" class="w-full px-4 py-3 border border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-700 dark:text-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 text-left" dir="ltr" placeholder="English description...">{{ old('description_en', $destination->description_en) }}</textarea>
            </div>

            <!-- Highlights -->
            <div class="mt-6">
                <label class="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-2">نکات برجسته</label>
                <textarea name="highlights" rows="3" class="w-full px-4 py-3 border border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-700 dark:text-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500">{{ old('highlights', $destination->highlights) }}</textarea>
            </div>
            ...
            <div class="mt-6">
                ...
                <div class="flex items-start gap-4">
                    <div class="w-40 h-40 bg-gray-100 dark:bg-gray-700 rounded-xl flex items-center justify-center overflow-hidden flex-shrink-0 border-2 border-gray-200 dark:border-gray-600">
                        <img id="featuredPreview" src="{{ asset('storage/' . $destination->featured_image) }}" class="w-full h-full object-cover">
                    </div>
                    <div class="flex-1">
                        ...
                        <input type="file" name="featured_image" id="featured_image" class="hidden" accept="image/*" onchange="previewImage(this, 'featuredPreview', null)">
                    </div>
                </div>
            </div>
            ...
            <div class="flex gap-6 mt-6 bg-gray-50 dark:bg-gray-700/50 rounded-xl p-4">
                <label class="flex items-center gap-3 cursor-pointer">
                    <input type="checkbox" name="is_trending" value="1" {{ $destination->is_trending ? 'checked' : '' }} class="w-5 h-5 rounded border-gray-300 dark:border-gray-600 text-amber-500">
                    <span class="text-sm font-bold text-gray-700 dark:text-gray-300">⭐ پرطرفدار</span>
                </label>
                <label class="flex items-center gap-3 cursor-pointer">
                    <input type="checkbox" name="is_active" value="1" {{ $destination->is_active ? 'checked' : '' }} class="w-5 h-5 rounded border-gray-300 dark:border-gray-600 text-green-500">
            ...
            <div class="flex gap-3 mt-8">
                ...
                <a href="{{ route('admin.destinations.index') }}" class="bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300 px-8 py-3 rounded-xl font-bold hover:bg-gray-200 dark:hover:bg-gray-600 transition-colors">انصراف</a>
            </div>
        </form>

        <form action="{{ route('admin.destinations.destroy', $destination) }}" method="POST" class="mt-6 pt-6 border-t border-gray-200 dark:border-gray-700">
            @csrf
            @method('DELETE')
