            ...
            <table class="w-full">
                <thead>
                    <tr class="bg-gray-50 dark:bg-gray-700/50">
                        ...
                        <th class="text-right py-4 px-4 text-xs font-bold text-gray-400 dark:text-gray-500 uppercase">نوع
                        </th>
                        <th class="text-right py-4 px-4 text-xs font-bold text-gray-400 dark:text-gray-500 uppercase">مورد
                        </th>
                        <th class="text-right py-4 px-4 text-xs font-bold text-gray-400 dark:text-gray-500 uppercase">امتیاز
                        </th>
                        <th class="text-right py-4 px-4 text-xs font-bold text-gray-400 dark:text-gray-500 uppercase">نظر
                        </th>
                        ...
                        <tr class="hover:bg-gray-50/50 dark:hover:bg-gray-700/50">
                            <td class="py-4 px-4">
                                <div class="flex items-center gap-2">
                                    <div
                                        class="w-8 h-8 bg-blue-100 dark:bg-blue-900/30 rounded-full flex items-center justify-center text-blue-600 dark:text-blue-400 font-bold text-xs">
                                        {{ mb_substr($review->user->name ?? '?', 0, 1) }}
                                    </div>
                                    <span
                                        class="text-sm font-medium text-gray-700 dark:text-gray-200">{{ $review->user->name ?? 'ناشناس' }}</span>
                                </div>
                            </td>
                            <td class="py-4 px-4">
                                <span
                                    class="px-2 py-1 rounded-lg text-xs font-bold bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300">
                                    @php
                            ...
                            <td class="py-4 px-4">
                                <p class="text-sm text-gray-600 dark:text-gray-300 max-w-xs">
                                    {{ Str::limit($review->comment, 80) }}</p>
                            </td>
                            <td class="py-4 px-4">
                                <span
                                    class="px-3 py-1 rounded-lg text-xs font-bold {{ $review->is_approved ? 'bg-green-50 dark:bg-green-900/30 text-green-600 dark:text-green-400' : 'bg-yellow-50 dark:bg-yellow-900/30 text-yellow-600 dark:text-yellow-400' }}">
                                    {{ $review->is_approved ? '✅ تایید شده' : '⏳ در انتظار' }}
                                </span>
                            </td>
                            <td class="py-4 px-4 text-sm text-gray-400 dark:text-gray-500">
                                {{ $review->created_at->format('Y-m-d') }}</td>
                            <td class="py-4 px-4">
                                <div class="flex gap-2">
                                    <form action="{{ route('admin.reviews.approve', $review) }}" method="POST">
                                        @csrf @method('PATCH')
                                        <button
                                            class="p-2 rounded-lg text-xs font-bold {{ $review->is_approved ? 'bg-yellow-50 dark:bg-yellow-900/30 text-yellow-600 dark:text-yellow-400 hover:bg-yellow-100 dark:hover:bg-yellow-900/50' : 'bg-green-50 dark:bg-green-900/30 text-green-600 dark:text-green-400 hover:bg-green-100 dark:hover:bg-green-900/50' }}"
                                            title="{{ $review->is_approved ? 'لغو تایید' : 'تایید' }}">
