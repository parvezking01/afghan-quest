...
@section('content')
    ...
    <div class="flex items-center justify-between mb-6">
        <p class="text-gray-500 dark:text-gray-400">کل: <span
                class="font-bold text-gray-700 dark:text-white">{{ $packages->total() }}</span> پکیج</p>
                ...
                <thead>
                    <tr class="bg-gray-50 dark:bg-gray-700/50">
                        ...
                        <th class="text-right py-4 px-6 text-xs font-bold text-gray-400 dark:text-gray-500 uppercase">نام
                            پکیج</th>
                        <th class="text-right py-4 px-6 text-xs font-bold text-gray-400 dark:text-gray-500 uppercase">نوع
                        </th>
                        <th class="text-right py-4 px-6 text-xs font-bold text-gray-400 dark:text-gray-500 uppercase">مدت
                        </th>
                        <th class="text-right py-4 px-6 text-xs font-bold text-gray-400 dark:text-gray-500 uppercase">قیمت
                        </th>
                        <th class="text-right py-4 px-6 text-xs font-bold text-gray-400 dark:text-gray-500 uppercase">ظرفیت
                        </th>
                        ...
                        <tr class="hover:bg-gray-50/50 dark:hover:bg-gray-700/50 transition-colors">
                            ...
                            <td class="py-4 px-6">
                                ...
                                <p class="text-xs text-gray-400 dark:text-gray-500">
                                    {{ Str::limit($package->description, 50) }}</p>
                            </td>
                            <td class="py-4 px-6">
                                <span
                                    class="bg-purple-50 dark:bg-purple-900/30 text-purple-600 dark:text-purple-400 px-3 py-1 rounded-lg text-xs font-bold">
                            ...
                            <td class="py-4 px-6">
                                <p class="font-bold text-gray-700 dark:text-white">{{ number_format($package->price) }} اف
                                </p>
                                @if ($package->discount_price)
                                    <p class="text-xs text-red-500 dark:text-red-400 line-through">
                                        {{ number_format($package->discount_price) }} اف</p>
                                @endif
                            </td>
                            <td class="py-4 px-6 text-sm text-gray-600 dark:text-gray-300">{{ $package->max_travelers }}
                                نفر</td>
                            <td class="py-4 px-6">
                            ...
                            <td class="py-4 px-6">
                                <span
                                    class="px-3 py-1 rounded-lg text-xs font-bold {{ $package->is_active ? 'bg-green-50 dark:bg-green-900/30 text-green-600 dark:text-green-400' : 'bg-gray-100 dark:bg-gray-700 text-gray-400 dark:text-gray-500' }}">
                    ...
                    @empty
                        <tr>
                            <td colspan="9" class="text-center py-12">
                                ...
                                <h3 class="text-lg font-bold text-gray-600 dark:text-gray-300 mb-2">هیچ پکیجی وجود ندارد
                                </h3>
                                <a href="{{ route('admin.packages.create') }}"
