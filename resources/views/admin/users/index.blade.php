...
<div class="grid grid-cols-3 gap-4 mb-6">
    ...
    <div class="bg-white dark:bg-gray-800 rounded-2xl p-5 border border-gray-100 dark:border-gray-700 text-center">
        <p class="text-3xl font-black text-blue-600 dark:text-blue-400">{{ \App\Models\User::where('role', 'tourist')->count() }}</p>
        <p class="text-gray-400 dark:text-gray-500 text-sm">🧳 گردشگران</p>
    </div>
    <div class="bg-white dark:bg-gray-800 rounded-2xl p-5 border border-gray-100 dark:border-gray-700 text-center">
        <p class="text-3xl font-black text-yellow-600 dark:text-yellow-400">{{ \App\Models\User::where('role', 'hotel_owner')->count() }}</p>
        ...
        <table class="w-full">
            <thead>
                <tr class="bg-gray-50 dark:bg-gray-700/50">
                    ...
                    <th class="text-right py-4 px-4 text-xs font-bold text-gray-400 dark:text-gray-500 uppercase">ایمیل</th>
                    <th class="text-right py-4 px-4 text-xs font-bold text-gray-400 dark:text-gray-500 uppercase">نقش</th>
                    <th class="text-right py-4 px-4 text-xs font-bold text-gray-400 dark:text-gray-500 uppercase">شماره</th>
                ...
                <tr class="hover:bg-gray-50/50 dark:hover:bg-gray-700/50">
                    <td class="py-4 px-4">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p class="text-xs text-gray-400 dark:text-gray-500">{{ $user->whatsapp ?? $user->phone }}</p>
                            </div>
                        </div>
                    </td>
                    ...
                    <td class="py-4 px-4">
                        <span class="px-3 py-1 rounded-lg text-xs font-bold
                            ...
                            @elseif($user->role === 'hotel_owner') 🏨 مالک
                            @else 🧳 گردشگر
                            @endif
                        </span>
                    </td>
                    ...
                    <td class="py-4 px-4">
                        ...
                        @else
                            <span class="bg-green-50 dark:bg-green-900/30 text-green-600 dark:text-green-400 px-3 py-1 rounded-lg text-xs font-bold">✅ مدیر</span>
                        @endif
                    </td>
                    <td class="py-4 px-4 text-sm text-gray-400 dark:text-gray-500">{{ $user->created_at->format('Y-m-d') }}</td>
                    <td class="py-4 px-4">
                        @if($user->id !== auth()->id())
                        <form action="{{ route('admin.users.destroy', $user) }}" method="POST" onsubmit="return confirm('آیا از حذف این کاربر مطمئن هستید؟')">
                            @csrf
                            @method('DELETE')
