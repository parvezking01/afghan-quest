...
@section('content')
    ...
    <div class="flex items-center justify-between mb-6">
        ...
        <button onclick="document.getElementById('addRoomForm').classList.toggle('hidden')"
        ...
        <form action="{{ route('hotel_owner.rooms.store', $hotel) }}" method="POST">
            ...
            <div class="grid md:grid-cols-2 gap-4 mb-4">
                <div>
                    ...
                    <input type="text" name="room_type"
                        ...
                        placeholder="مثال: یک نفره، دو نفره، سوئیت" required>
                </div>
                <div>
            ...
            <div class="mb-4">
                ...
                <textarea name="description" rows="2"
                    class="w-full px-4 py-2 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500"></textarea>
            </div>
            <button type="submit"
                class="bg-green-500 text-white px-6 py-2 rounded-xl font-bold hover:bg-green-600 transition-colors">
                ...
                <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                    <div>
                        ...
                        <div class="flex gap-4 mt-2 text-sm text-gray-500">
                            ...
                            <span><i class="fas fa-door-open ms-1"></i> کل: {{ $room->total_rooms }} | موجود:
                                {{ $room->available_rooms }}</span>
                            <span class="font-bold text-gray-700">{{ number_format($room->price_per_night) }} اف / شب</span>
                        </div>
                    </div>
                    <div class="flex gap-2">
                        <button
                            onclick="editRoom({{ $room->id }}, '{{ $room->room_type }}', {{ $room->price_per_night }}, {{ $room->capacity }}, {{ $room->total_rooms }}, {{ $room->available_rooms }}, '{{ $room->description }}')"
                            class="bg-blue-50 text-blue-600 px-4 py-2 rounded-lg text-sm font-bold hover:bg-blue-100">
                            <i class="fas fa-edit"></i>
                        </button>
                        <form action="{{ route('hotel_owner.rooms.destroy', ['hotel' => $hotel, 'room' => $room]) }}"
    ...
    <a href="{{ route('hotel_owner.hotels.index') }}"
        class="inline-flex items-center gap-2 text-gray-500 hover:text-gray-700 mt-6">
