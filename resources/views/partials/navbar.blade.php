<nav id="navbar" class="fixed top-0 left-0 right-0 z-50 bg-white/70 dark:bg-gray-900/80 backdrop-blur-xl transition-all duration-500 py-3">
    <div class="container mx-auto px-4">
        <div class="flex justify-between items-center">
            ...
            <div class="flex items-center gap-8">
                ...
                <div class="hidden lg:flex items-center gap-5">
                    <a href="{{ url('/') }}" class="font-medium text-sm text-gray-700 dark:text-gray-300 hover:text-primary-500 dark:hover:text-gold-400 transition-colors">
                        {{ app()->getLocale() === 'en' ? 'Home' : 'صفحه اصلی' }}
                    </a>
                    <a href="{{ route('provinces.index') }}" class="font-medium text-sm text-gray-700 dark:text-gray-300 hover:text-primary-500 dark:hover:text-gold-400 transition-colors">
            ...
            <div class="hidden lg:flex items-center gap-1">
                ...
                <div x-data="{ open: false }" class="relative">
                    <button @click="open = !open" class="text-gray-500 dark:text-gray-300 hover:text-primary-500 transition-colors p-2 text-lg">
                ...
                <div class="flex items-center gap-0">
                    <div class="flex items-center gap-0 border border-gray-200 dark:border-gray-600 rounded-full overflow-hidden">
                    ...
                    <button onclick="toggleDarkMode()" class="text-gray-500 dark:text-gray-300 hover:text-yellow-500 transition-colors p-2 text-lg">
                ...
                @auth
                    @if(auth()->user()->isAdmin())
                        <a href="{{ route('admin.dashboard') }}" class="gradient-btn text-white px-4 py-2 rounded-full text-xs font-bold">
                            {{ app()->getLocale() === 'en' ? 'Admin' : 'مدیریت' }}
                        </a>
                    @elseif(auth()->user()->isHotelOwner())
                        <a href="{{ route('hotel_owner.dashboard') }}" class="gradient-btn text-white px-4 py-2 rounded-full text-xs font-bold">
                            {{ app()->getLocale() === 'en' ? 'Hotel Panel' : 'پنل هتل' }}
                        </a>
                    @else
                        <a href="{{ route('tourist.dashboard') }}" class="gradient-btn text-white px-4 py-2 rounded-full text-xs font-bold">
                ...
                @else
                    <a href="{{ route('login') }}" class="border-2 border-primary-500 text-primary-500 dark:text-primary-400 dark:border-primary-400 px-4 py-2 rounded-full text-xs font-bold hover:bg-primary-500 hover:text-white transition-all">
                        {{ app()->getLocale() === 'en' ? 'Login' : 'ورود' }}
                    </a>
                    <a href="{{ route('register') }}" class="gradient-gold text-primary-900 px-4 py-2 rounded-full text-xs font-bold ml-1">
            ...
            <!-- Mobile Menu Button -->
            <button onclick="document.getElementById('mobile-menu').classList.toggle('hidden')" class="lg:hidden text-2xl text-gray-700 dark:text-gray-300">
        ...
        <div id="mobile-menu" class="hidden lg:hidden mt-4 pb-4">
            <div class="flex flex-col gap-3 bg-white dark:bg-gray-800 rounded-2xl p-4 shadow-2xl">
                ...
                <form action="{{ route('search') }}" method="GET">
                    <div class="relative">
                        <input type="text" name="q" placeholder="{{ app()->getLocale() === 'en' ? 'Search...' : 'جستجو...' }}"
                        ...
                        <button type="submit" class="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400"><i class="fas fa-search"></i></button>
                    </div>
                </form>
                ...
                <div class="flex gap-1 justify-center">
                    <a href="{{ route('language.switch', 'fa') }}" class="px-4 py-1 text-xs font-bold rounded-full {{ app()->getLocale() === 'fa' ? 'bg-primary-500 text-white' : 'bg-gray-100 dark:bg-gray-700' }}">FA</a>
                    <a href="{{ route('language.switch', 'en') }}" class="px-4 py-1 text-xs font-bold rounded-full {{ app()->getLocale() === 'en' ? 'bg-primary-500 text-white' : 'bg-gray-100 dark:bg-gray-700' }}">EN</a>
                </div>

                @auth
                    <a href="{{ route('tourist.dashboard') }}" class="gradient-btn text-white text-center py-2 rounded-xl font-bold text-sm">{{ app()->getLocale() === 'en' ? 'Dashboard' : 'داشبورد' }}</a>
                @else
                    <a href="{{ route('login') }}" class="gradient-btn text-white text-center py-2 rounded-xl font-bold text-sm">{{ app()->getLocale() === 'en' ? 'Login' : 'ورود' }}</a>
                    <a href="{{ route('register') }}" class="gradient-gold text-primary-900 text-center py-2 rounded-xl font-bold text-sm">{{ app()->getLocale() === 'en' ? 'Register' : 'ثبت نام' }}</a>
                @endauth
            </div>
        </div>
    </div>
