    ...
    <section class="relative py-20"
        ...
        <div class="container mx-auto px-4 text-center relative z-10">
            <h1 class="text-4xl lg:text-5xl font-black text-white mb-4">
                {{ app()->getLocale() === 'en' ? 'Provinces of Afghanistan' : 'ولایات افغانستان' }}</h1>
            <p class="text-xl text-gray-300">
                {{ app()->getLocale() === 'en' ? 'Explore the 34 beautiful provinces of Afghanistan' : 'کاوش در ۳۴ ولایت زیبای افغانستان' }}
            </p>
        </div>
    </section>
                    ...
                    <a href="{{ route('provinces.show', $province->slug) }}"
                        ...
                        <div class="p-5">
                            <p class="text-gray-600 dark:text-gray-300 text-sm leading-relaxed">
                                {{ Str::limit(locale_field($province, 'description'), 100) }}</p>
                            <div class="flex items-center gap-4 mt-4 text-xs text-gray-500 dark:text-gray-400">
                                <span><i class="fas fa-landmark text-blue-500 ms-1"></i>
                                    {{ $province->destinations_count ?? 0 }}
                                    {{ app()->getLocale() === 'en' ? 'Destinations' : 'مقصد' }}</span>
                                <span><i class="fas fa-hotel text-emerald-500 ms-1"></i> {{ $province->hotels_count ?? 0 }}
                                    {{ app()->getLocale() === 'en' ? 'Hotels' : 'هتل' }}</span>
                            </div>
                        </div>
                    </a>
                @endforeach
