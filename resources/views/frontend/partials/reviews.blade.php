    ...
    <form action="{{ route('reviews.store') }}" method="POST" class="mb-6 bg-gray-50 rounded-xl p-4">
        ...
        <div class="mb-3">
            ...
            <div class="flex gap-1 text-2xl star-container" style="direction: ltr; justify-content: flex-end;"
                ...
                <span class="star-item cursor-pointer text-gray-300" data-value="5" onclick="selectStar(this)" onmouseenter="hoverStars(this)"><i class="fas fa-star"></i></span>
                <span class="star-item cursor-pointer text-gray-300" data-value="4" onclick="selectStar(this)" onmouseenter="hoverStars(this)"><i class="fas fa-star"></i></span>
                <span class="star-item cursor-pointer text-gray-300" data-value="3" onclick="selectStar(this)" onmouseenter="hoverStars(this)"><i class="fas fa-star"></i></span>
                <span class="star-item cursor-pointer text-gray-300" data-value="2" onclick="selectStar(this)" onmouseenter="hoverStars(this)"><i class="fas fa-star"></i></span>
                <span class="star-item cursor-pointer text-gray-300" data-value="1" onclick="selectStar(this)" onmouseenter="hoverStars(this)"><i class="fas fa-star"></i></span>
            </div>
            <input type="hidden" name="rating" id="ratingInput-{{ $reviewable->id }}" value="0" required>
        </div>

        <div class="mb-3">
            <textarea name="comment" rows="3" class="w-full px-4 py-3 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="تجربه خود را بنویسید..." required></textarea>
        </div>

        <button type="submit" class="bg-blue-500 text-white px-6 py-2 rounded-xl font-bold hover:bg-blue-600 transition-colors">
            <i class="fas fa-paper-plane ms-1"></i> ثبت نظر
        </button>
    </form>
    @else
    <div class="bg-gray-50 rounded-xl p-4 text-center mb-6">
        <p class="text-gray-500">برای ثبت نظر <a href="{{ route('login') }}" class="text-blue-500 font-bold">وارد شوید</a></p>
    </div>
    @endauth
