        ...
        <div class="container mx-auto px-4 max-w-2xl">
            <div class="bg-white rounded-2xl shadow-sm p-8">
                ...
                <div class="bg-blue-50 rounded-xl p-4 mb-6">
                    ...
                    <p class="text-sm text-blue-600">{{ $package->duration_days }} روز / {{ $package->duration_nights }} شب |
                        {{ number_format($package->final_price) }} اف</p>
                </div>
                ...
                <form action="{{ route('booking.package.store') }}" method="POST">
                    ...
                    <div class="grid md:grid-cols-2 gap-4 mb-4">
                        <div>
                            ...
                            <input type="date" name="travel_date" value="{{ old('travel_date', date('Y-m-d')) }}"
                                min="{{ date('Y-m-d') }}"
                                class="w-full px-4 py-3 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500"
                                required>
                        </div>
                        <div>
                            ...
                            <input type="number" name="number_of_travelers" value="{{ old('number_of_travelers', 1) }}"
                                min="1" max="{{ $package->max_travelers }}"
                                class="w-full px-4 py-3 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500"
                                required>
                        </div>
                    </div>
