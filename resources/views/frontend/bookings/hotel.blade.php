...
@section('title', 'رزرو هوتل: ' . $hotel->name)
    ...
    <section class="py-16 bg-gray-50">
        <div class="container mx-auto px-4 max-w-2xl">
            <div class="bg-white rounded-2xl shadow-sm p-8">
                <h2 class="text-2xl font-black text-gray-800 mb-6">📋 فرم رزرو هوتل</h2>
                ...
                <form action="{{ route('booking.hotel.store') }}" method="POST">
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" name="guest_name" value="{{ auth()->user()->name ?? old('guest_name') }}"
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" name="whatsapp_number"
                            value="{{ auth()->user()->whatsapp ?? (auth()->user()->phone ?? old('whatsapp_number')) }}"
                            class="w-full px-4 py-3 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 text-left"
                        ...
                        <select name="room_id" id="roomSelect"
                            ...
                            @foreach ($hotel->rooms as $room)
                                <option value="{{ $room->id }}" data-price="{{ $room->price_per_night }}"
                                    ...
                                    {{ $room->room_type }} - {{ number_format($room->price_per_night) }} اف/شب
                                    ({{ $room->available_rooms }} موجود)
                                </option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="grid md:grid-cols-2 gap-4 mb-4">
                        <div>
                            ...
                            <input type="date" name="check_in_date" id="checkInDate"
                                value="{{ old('check_in_date', date('Y-m-d')) }}" min="{{ date('Y-m-d') }}"
                                class="w-full px-4 py-3 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500"
                                required onchange="calculateTotal()">
                        </div>
                        <div>
                    ...
                    <div class="mb-4">
                        ...
                        <input type="number" name="number_of_rooms" id="numberOfRooms"
                            value="{{ old('number_of_rooms', 1) }}" min="1"
                            class="w-full px-4 py-3 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500"
                            required onchange="calculateTotal()">
                    ...
                    <div class="mb-6">
                        ...
                        <textarea name="guest_message" rows="3"
                            class="w-full px-4 py-3 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500"
                            placeholder="توضیحات یا سوالات خود را بنویسید...">{{ old('guest_message') }}</textarea>
                    </div>

                    <button type="submit"
                        class="w-full bg-green-500 text-white py-4 rounded-xl font-bold text-lg hover:bg-green-600 transition-all shadow-lg">
    ...
    <script>
        function calculateTotal() {
            ...
            if (numberOfRooms > availableRooms) {
                availabilityMessage.innerHTML = '<span class="text-red-500">❌ فقط ' + availableRooms +
                    ' اتاق موجود است!</span>';
                document.getElementById('numberOfRooms').value = availableRooms;
                return;
            } else {
                availabilityMessage.innerHTML = '<span class="text-green-500">✅ ' + availableRooms +
                    ' اتاق موجود است</span>';
            }
