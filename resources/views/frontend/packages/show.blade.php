...
<section class="relative py-24" style="background: linear-gradient(rgba(26, 26, 46, 0.85), rgba(22, 33, 62, 0.9)), url('{{ $package->featured_image ? asset('storage/' . $package->featured_image) : 'https://images.unsplash.com/photo-1599070292747-ae92ea606fcf?w=1920' }}') center/cover;">
    <div class="container mx-auto px-4 text-center relative z-10">
        ...
        <div class="flex justify-center gap-4 text-white">
            <span><i class="far fa-clock ms-1"></i> {{ $package->duration_days }} {{ app()->getLocale() === 'en' ? 'Days' : 'روز' }} / {{ $package->duration_nights }} {{ app()->getLocale() === 'en' ? 'Nights' : 'شب' }}</span>
            <span><i class="fas fa-user ms-1"></i> {{ $package->max_travelers }} {{ app()->getLocale() === 'en' ? 'People' : 'نفر' }}</span>
            @if($package->includes_guide)
            <span><i class="fas fa-user-tie ms-1"></i> {{ app()->getLocale() === 'en' ? 'With Guide' : 'با راهنما' }}</span>
            @endif
        </div>
    </div>
</section>
    ...
    <div class="container mx-auto px-4">
        <div class="grid lg:grid-cols-3 gap-8">
            <div class="lg:col-span-2">
                ...
                <div class="bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-sm mb-6">
                    ...
                    <div class="grid md:grid-cols-2 gap-4">
                        ...
                        <div class="flex items-center gap-3 p-4 bg-gray-50 dark:bg-gray-700 rounded-xl hover:bg-gray-100 dark:hover:bg-gray-600 transition-colors">
                            <img src="{{ $dest->featured_image ? asset('storage/' . $dest->featured_image) : 'https://via.placeholder.com/60' }}" class="w-14 h-14 rounded-xl object-cover">
                            <div>
                                <p class="font-bold text-gray-800 dark:text-white">{{ locale_field($dest, 'name') }}</p>
                                <p class="text-xs text-gray-400 dark:text-gray-500">{{ locale_field($dest->province, 'name') }}</p>
                            </div>
                        </div>
                        @endforeach
                    </div>
                ...
                <div class="bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-sm mb-6">
                    <h3 class="text-xl font-black text-gray-800 dark:text-white mb-4">{{ app()->getLocale() === 'en' ? '✅ Included Services' : '✅ خدمات شامل' }}</h3>
                    <ul class="space-y-2">
                        @foreach(json_decode($package->included_services) as $service)
                        <li class="flex items-center gap-2 text-gray-600 dark:text-gray-300"><i class="fas fa-check-circle text-green-500"></i> {{ $service }}</li>
                        @endforeach
                    </ul>
                </div>
                @endif
                ...
                <div class="bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-sm mb-6">
                    <h3 class="text-xl font-black text-gray-800 dark:text-white mb-4">{{ app()->getLocale() === 'en' ? '❌ Excluded Services' : '❌ خدمات شامل نمی‌شود' }}</h3>
                    <ul class="space-y-2">
                        @foreach(json_decode($package->excluded_services) as $service)
                        <li class="flex items-center gap-2 text-gray-600 dark:text-gray-300"><i class="fas fa-times-circle text-red-500"></i> {{ $service }}</li>
                        @endforeach
                    </ul>
                </div>
                @endif
                ...
                <div class="bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-sm mb-6">
                    <h3 class="text-xl font-black text-gray-800 dark:text-white mb-4">{{ app()->getLocale() === 'en' ? '📅 Itinerary' : '📅 برنامه سفر' }}</h3>
                    <div class="space-y-3">
                        @foreach(json_decode($package->itinerary) as $day)
                        <div class="flex items-start gap-3 p-3 bg-gray-50 dark:bg-gray-700 rounded-xl">
                            <span class="bg-blue-500 text-white w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold flex-shrink-0">{{ $loop->iteration }}</span>
            ...
            <div>
                <div class="bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-sm sticky top-24">
                    <div class="text-center mb-6 bg-gray-50 dark:bg-gray-700 rounded-xl p-4">
                        ...
                        <span class="bg-red-100 dark:bg-red-900/30 text-red-600 dark:text-red-400 text-xs px-3 py-1 rounded-full font-bold">{{ app()->getLocale() === 'en' ? 'Special Offer' : 'تخفیف ویژه' }}</span>
                        @else
                        <p class="text-4xl font-black text-gray-800 dark:text-white">{{ number_format($package->price) }} اف</p>
                        @endif
                    </div>
