...
@section('content')
    ...
    <h2 class="text-2xl font-black text-gray-800 dark:text-white mb-6">📋
        {{ app()->getLocale() === 'en' ? 'My Bookings' : 'رزروهای من' }}</h2>
                    ...
                    <thead>
                        <tr class="bg-gray-50 dark:bg-gray-700/50">
                            <th class="text-right py-4 px-4 text-xs font-bold text-gray-400 dark:text-gray-500 uppercase">
                                {{ app()->getLocale() === 'en' ? 'Booking #' : 'کد رزرو' }}</th>
                            <th class="text-right py-4 px-4 text-xs font-bold text-gray-400 dark:text-gray-500 uppercase">
                            ...
                            <th class="text-right py-4 px-4 text-xs font-bold text-gray-400 dark:text-gray-500 uppercase">
                                {{ app()->getLocale() === 'en' ? 'WhatsApp' : 'واتساپ' }}</th>
                        </tr>
                    </thead>
                        ...
                        @foreach ($bookings as $booking)
                            ...
                            {{ $booking->status === 'completed' ? 'bg-purple-50 dark:bg-purple-900/30 text-purple-600 dark:text-purple-400' : '' }}">
                                        ...
                                        @elseif($booking->status === 'contacted')
                                            {{ app()->getLocale() === 'en' ? '📞 Contacted' : '📞 تماس گرفته شد' }}
    ...
    @else
        <div
            ...
            <h3 class="text-xl font-black text-gray-600 dark:text-gray-300 mb-2">
                {{ app()->getLocale() === 'en' ? 'No Bookings Yet' : 'هیچ رزروی ندارید' }}</h3>
            ...
            <a href="{{ route('packages.index') }}"
                class="bg-blue-500 text-white px-6 py-3 rounded-xl font-bold hover:bg-blue-600 transition-colors inline-block">
